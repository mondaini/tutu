...
exports.purgeApi = function () {
  return apigateway.purgeApi();
}

exports.createDeployment = () => {
  logger.status('Creating a deployment')
...
let putIntegrationAndMethodResponse = (response) => {
  logger.status('Deploying METHOD Integration')
  return apigateway.putMethodResponse(response)
    .then((err) => {
      return apigateway.putIntegrationResponse(response)
    })
}
